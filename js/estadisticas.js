document.addEventListener('DOMContentLoaded', () => {
    // 1. Recuperar ID y Nombre guardados
...
async function cargarEstadisticas(id) {
    try {
        ...
        if (response.ok) {
            // FIX: Leer como texto primero para evitar errores si la respuesta viene vacía
            const text = await response.text(); 
            const stats = text ? JSON.parse(text) : null; 
            
            // Si el usuario es nuevo o no tiene tareas terminadas
            if (!stats) {
            ...
            // Usamos los campos originales de tu backend actual
            ...
            // Calcular eficiencia visualmente
...
function calcularEficiencia(sesiones, interrupciones) {
    ...
    if (sesiones > 0) {
        // Regla simple: Cada interrupción por sesión resta un poco de eficiencia
...
function animarNumero(id, valorFinal) {
    ...
    // Si quieres el efecto de conteo, descomenta esto. Si no, déjalo directo:
